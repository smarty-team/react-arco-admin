...
import { ResponseError, useSms, useSmsLogin } from "../service/user";
...
import { setLoginState } from "@/app/store/authSlice";
...
export default function verifyCode() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm({ values: { smsCode: params.get("smsCode") } });
              ...
              <div className="form-control">
                ...
                <label className="label">
                  <span className="text text-gray-400">
                    ...
                    {/* 测试显示结果 */}
                    测试系统，短信验证码为 {params.get("smsCode")}
                  </span>
                </label>
              </div>
              <div className="form-control">
                <div className="flex flex-row">
                  <input
                    ...
                    autoFocus
                    onKeyDown={onkeydown}
                    defaultValue={params.get('smsCode') || ''}
                    {...register("smsCode", {
                      required: true,
                      pattern: /^\d{4}$/,
                    })}
